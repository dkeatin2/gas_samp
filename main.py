...
columns = ['Date', 'Time UTC', 'Latitude+', 'Latitude-', 'Data String Indicator', 'DSP counter', 'CO2 PPMV', 'N2O PPBV', 'Pressure (Torr)', 'Temp (K)']
	...
	while True:
		try:
			...
			if count >= autosave_interval:
				df.to_csv(f'autosave_{int(time.time())}_{csv_file}', index=False)
				df = pd.DataFrame(columns=columns)
				count = 0
		except Exception as e:
